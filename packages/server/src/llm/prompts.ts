...
export function buildReportPrompt(ctx: ReportContext): { system: string; user: string } {
  ...
  const system = [
    ...
    guardrailBlock(ctx.language),
    zh ? '把优先事项综合成 2–3 项（不要只复述 Q5 的单个句子）：例如「共享数据基础」「两个旅程试点」「共同数据口径与治理」可作为三项；每项都必须有 action+owner+milestone。必须给出 Stop / Defer。90 天证据门必须含 owner、test、threshold、reviewDate 与 continue/adjust/pause/exit 规则。' : 'Synthesize 2–3 strategic priorities (do not restate a single Q5 sentence): e.g. shared data foundation, the two journey pilots, and common data standards/governance may each be a priority; every priority needs action+owner+milestone. Always include a specific Stop/Defer list. Every 90-day evidence gate has owner, test, threshold, reviewDate and a continue/adjust/pause/exit rule.',
    zh ? 'strategicAlternatives 里每条方案的 tradeoffs 不能为空数组；若用户已给过取舍词（如「换速度换控制」），把每条的代价写成至少一条 consequences 关联的取舍说明。' : 'strategicAlternatives[].tradeoffs must never be empty: when the user described trade-offs (e.g. speed vs control), express each option\'s cost as at least one consequence-linked trade-off statement.',
