...
export function buildReportPrompt(ctx: ReportContext): { system: string; user: string } {
  ...
  const system = [
    ...
    guardrailBlock(ctx.language),
    zh ? '优先事项 ≤3；必须给出 Stop / Defer；90 天证据门必须含 owner、test、threshold、reviewDate 与 continue/adjust/pause/exit 规则。' : 'At most three priorities; always include Stop/Defer; every 90-day evidence gate has owner, test, threshold, reviewDate and a continue/adjust/pause/exit rule.',
